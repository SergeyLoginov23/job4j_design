...
class CSVReaderTest {
    ...
    @Test
    void whenInvalidArgsThenException() {
        assertThatThrownBy(() -> ArgsName.of(new String[]{
                "-path=", "-delimiter=;", "-out=target.csv", "filter=age,name"
        }))
                .isInstanceOf(IllegalArgumentException.class);
    }

    @Test
    void whenFilterHasNotExistingColumnThenException(@TempDir Path folder) throws Exception {
        String data = String.join(
                System.lineSeparator(),
                "name;age;city",
                "Tom;20;NY"
